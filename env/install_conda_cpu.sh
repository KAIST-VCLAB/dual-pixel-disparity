...
if [[ "$(uname -s)" =~ "Linux" ]]; then
    ...
    echo "Detected: [Linux]"
    echo "----------------------------------------------------------------"
    PYTORCH_INSTALL="pip install --no-input --timeout 180 --extra-index-url https://download.pytorch.org/whl/cpu \
        torch==1.11.0+cpu \
    ...
    echo "Detected: [Mac]"
    echo "----------------------------------------------------------------"
    PYTORCH_INSTALL="pip install --no-input --timeout 180 \
        torch==1.11.0 \
        torchvision==0.12.0"
    ...
    echo "Only Linux / Mac / Windows 64bit is supported"
    exit
fi
